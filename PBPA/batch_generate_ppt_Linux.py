import numpy as np
from numpy import unique, nonzero, shape, mean, std, zeros,ones, arange, mod, asarray,c_,squeeze, round,argsort,sort,size,copy
...
from PBPA.report_plugins import *
import glob
import pickle
...
with open(analysis_result_file, 'rb') as f:
    cfg = pickle.load(f)
    req_size_record = cfg['req_size_record']
    time_wow_record = cfg['time_wow_record']
    lists_action = cfg['lists_action']
    seek_dist_record = cfg['seek_dist_record']
    idle_queue_record = cfg['idle_queue_record']
    basic_info = cfg['basic_info']
    stack_wow_record = cfg['stack_wow_record']
    freq_wow_record = cfg['freq_wow_record']

# generate the ppt with template
ppt_template = _get_module_path('./Data/workload.pptx')
...
# print('idx',idx)
# print('sum',sum(idle_queue_record.idle_time_array[idx,1]))
temp_str21=(('Total effective idle time (>0.1ms)='+"{0:.3f}".format(sum(idle_queue_record.idle_time_array[idx,1][0]))+'; Total idle time='+"{0:.3f}".format(sum(idle_queue_record.idle_time_array[:,1]))))
...
temp_str3=(('The minimun amplification ratio is '+"{0:.3f}".format(amp_ratio[0])+', if only the updated blocks in one request are written to the new places'))
...
idx=nonzero(idle_queue_record.idle_time_array[:,1] > 0.1)
temp_str3=(('Total effective idle time (>0.1s)='+"{0:.3f}".format(sum(idle_queue_record.idle_time_array[idx,1][0]))+'; Total idle time='+"{0:.3f}".format(sum(idle_queue_record.idle_time_array[:,1]))))
